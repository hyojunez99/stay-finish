# [프로젝트 이름]

> STAY(스테이) 실시간 주차 위치 파악 및 사용자의 피로도를 낮춘 방문 차량 관리

## 1. 프로젝트 소개

- **설명:** 기획부터 디자인, 개발까지 직접 참여하였으며, 입주민·사업자·관리자 역할 기반 UI와 주차 현황 확인 및 차량 등록 기능을 중심으로 한 주차 관리 시스템 프로젝트입니다. Supabase를 활용하여 방문 차량 등록, 장기 차량 관리 등 실제 서비스 흐름을 구현하였습니다.
- **진행 기간:** 2025.01.02 ~ 2025.01.13 (12일)
- **개발 인원:** FrontEnd 6인 (Team Project)
- **배포 링크:** [https://hyojunez99.github.io/stay-finish/]

## 2. 사용 기술 스택 (Tech Stack)

- **Language:** JavaScript (ES6+)
- **Framework:** React.js
- **Styling:** SCSS
- **Data Handling:** Custom Mock Data (JSON)
- **Design & Tool:** **Figma** Photoshop, Illustrator, Git, GitHub, GSAP, Sass, Supabase

## 3. 기획 및 디자인 (Planning & Design)

- **Tool:** Figma
- **Concept:** 사용자 직관성을 고려한 UI/UX 설계 및 와이어프레임 제작

## 4. 디렉토리 구조

```text
src
├── api             # Supabase 기반으로 관리자, 사용자, 주차, 정산 등 주요 기능의 데이터 처리를 담당하는 API 모음
│   
├── assets          # 프로젝트 전반에서 공통으로 사용하는 리소스를 관리
│     │   
│     ├── images    # 페이지 각 섹션에 사용되는 이미지 파일을 관리
│     │   
│     ├── scss      # 공통 스타일 정의를 위한 색상 변수, 폰트 스타일, 반응형 및 레이아웃 mixin, 전역 스타일을 관리
│     │   
│     └── svg       # 페이지에 사용되는 SVG 아이콘 및 이미지 파일을 관리
│     
├── components      # 공통 UI 및 서비스 핵심 기능 컴포넌트를 관리
│     │    
│     ├── common    # 프로젝트 전반에서 공통으로 사용되는 UI 및 스크립트 컴포넌트를 관리
│     │   
│     └── mode      # 차량 등록, 할인권 발급 및 정산 등 주차장 관리 핵심 기능 컴포넌트를 관리
│     
├── contexts        # 사용자, 관리자, 주차 현황 등 도메인별 상태 관리
│
├── layout          # 공통 페이지 레이아웃 관리
│
└── pages           # 실제 보여지는 페이지 컴포넌트 관리

## 5. 담당 역할

[Git 브랜치 관리 및 소스 코드 병합(Merge) 및 메인 화면 및 공통 UI 구현]
* 컴포넌트, 페이지, API, 스타일 파일을 역할별로 분리
* 공통 로직은 Context 및 공용 컴포넌트로 관리
* 재사용 가능한 구조를 고려한 폴더 구성
* 디자이너가 제공한 와이어프레임을 기준으로 화면 구현
* 공통 헤더 및 주요 섹션을 컴포넌트화하여 관리

[개발:  기능]
*



## 6. 주요 기능

사용자 역할별 화면 제공
- 입주민 / 상가 사용자 역할에 따라 다른 대시보드 화면 제공

공통 헤더 정보 조회
- 로그인 시 사용자 정보 자동 조회
- 새로고침 시에도 Context 기반으로 헤더 정보 유지
- 사용자 이름, 동·호수, 현재 상태 정보 표시

주차 할인권 발급 (상가)
- 상가 계정으로 주차 할인권 발급 가능
- 30분 단위로 할인권 수량 자동 계산
- 발급된 할인권은 누적 관리

주차 할인권 정산
- 발급된 할인권 수량 및 정산 금액 조회
- 정산 예정일 정보 제공
- 할인권 발급 내역을 기반으로 자동 금액 계산

방문 차량 등록
- 단기 및 장기 방문 차량 등록
- 방문 날짜 및 사유 입력 가능
- 등록 후 서버에 저장되어 주차 관리에 반영

날씨 정보 제공
- 대시보드 상단에서 현재 날씨 정보 제공

## 7. 트러블 슈팅

문제 1. 새로고침 시 사용자 정보가 사라지는 문제 (미해결)

상황: 로그인 직후에는 BusinessDashboard, ResidentDashboard에서 사용자 이름, 동·호수 등 헤더 정보가 정상적으로 표시됨
하지만 페이지 새로고침(F5) 시 사용자 정보를 불러오는 중... 문구만 계속 출력됨

과정: 새로고침 시 사용자 정보가 다시 로드되지 않는 문제를 해결하기 위해
profile.id가 준비되었을 때 헤더 정보를 다시 요청하도록
useEffect 의존성 배열을 다음과 같이 조정함.

useEffect(() => {
if (!profile?.id) return;
fetchHeader();
}, [profile?.id]);

해결되지 않은 이유 : 사용자 정보를 Context에 다시 복원하는 로직이 구현되어 있지 않아
헤더 정보가 끝까지 로딩되지 않는 상태가 발생함

## 8. 실행 화면
(구현한 화면 스크린샷 1장 첨부)
```
